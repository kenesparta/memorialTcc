...
\begin{document}
	...
	\newcommand*\tcc{
		...
		por um extrator de dados e um \emph{wrapper} que traduz os conteúdos da Web
		para tabelas do tipo relacionais. O extrator de dados das páginas
		Web do presente trabalho será inspirado no extrator deste artigo.
		
		
		\section{Fundamentação teórica}
		
		A seguir, apresentamos breves definições sobre os assuntos teóricos
		relacionados com a monografia, utilizando-se uma abordagem \emph{top-down}
		como pode-se observar na \cref{fig:organ_teorica}, e exibimos uma rede de
		Petri que representa a montagem teórica do presente trabalho, na qual o
		...
		(Evolutionary Computing\nomenclature[EC]{EC}{Evolutionary Computing}), sistemas
		difusos (Fuzzy Systems\nomenclature[FS]{FS}{Fuzzy Systems}) e raciocínio
		probabilístico (Probabilistic Reasoning\nomenclature[PR]{PR}{Probabilistic
		Reasoning}), como mostrado na \cref{fig:sc_comp}.
		...
		Saber em que momento precisa-se do ML é fundamental	porque justifica a
		utilização dos conceitos de ML no trabalho. Em
		\cite{shai_2014}, sugere-se que os conceitos de ML devem ser utilizados em
		tarefas realizadas por humanos ou animais. De acordo com isso, precisa-se do
		ML para poder modelar os algoritmos deste trabalho, de modo que utilizam-se 
		...
		por reforço, trata-se do modo de agir ou se comportar diante de uma situação
		...
		\begin{description}[style=multiline,leftmargin=2.5cm]
			...
			e iterativo, e possui cinco passos de desenvolvimento que se podem observar
			na \cref{fig:KDD}.
		...
		Para passar ao seguinte estado da rede de Petri estabelecida, de acordo com a
...
% @FR Explicar a diferença entre `Própria' e 'Minha organização' na tabela 5.1
% acima
		...
		explica-se que mineração na Web trata do processo de descoberta de informação
		útil e relevante a partir de estruturas de hiper-referências, conteúdo das
		...
		na Web mostradas a seguir, conforme a \cref{fig:Web_mining_taxonomia}:
		
		\begin{description}[style=multiline,leftmargin=2.5cm]
			
			\item [{Conteúdo}] Ou Web Content Mining (WUM {\it ou WCM?} @FR), é a
			extração de informação valiosa a partir de informação alocada em páginas Web.
		...
		veja-se na \cref{fig:arquivo_wiki}, utilizam-se as seguintes bibliotecas
		open source: Scipy, Numpy, Matplotlib e Jupyter. Uma breve explicação
		sobre o funcionamento daquelas bibliotecas é exposta em seguida:
		...
		\begin{description}[style=multiline,leftmargin=3.5cm]
			...
			bancos de dados de Web logs da Wikipédia e os que foram obtidos a
			través da mineração Web de modo que o “ruido” não desejado é eliminado.
			Também aplicam-se técnicas de agrupamento de dados, tudo isso é para
			poder otimizar o treinamento da rede neural que é modelada no seguinte
			passo.
		...
		da aplicação Web proposta no trabalho, veja-se na \cref{fig:petri_funcionamento}.
		...
		em vários sites (relacionados com artigos acadêmicos), essa busca
		é realizada utilizando os métodos de Web scraping. Para extrair dados
		...
		Web são armazenados em um banco de dados não relacional (neste caso
		...
		Finalmente, a resposta preparada é mostrada para o usuário, dita resposta
