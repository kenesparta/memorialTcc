	...
	\newcommand*\tcc{
		...
		\begin{flushright}
			\begin{minipage}[t]{0.48\columnwidth}%
				...
				do grau de Engenheiro da Computação na Universidade Federal do Ceará,
				\emph{campus} Sobral.%
			\end{minipage}
			\par\end{flushright}
		...
		\noindent \begin{flushleft}
			...
			\begin{tabular}{l}
				...
				\hline 
				Prof. Me. Fernando Rodrigues de Almeida Júnior\tabularnewline
				\tabularnewline
		...
		do tempo, relacionados ao servidor que estão em ordem cronologicamente
		...
		Petri, de modo que os estados são as áreas da Ciência da Computação
		...
		Soft Computing (SC\nomenclature[SC]{SC}{Soft Computing}), que é uma
		área das Ciências da Computação que permite resolver problemas com
		uma abordagem orientada à tolerância de imprecisões. Em seguida, aborda-se
		a teoria relacionada com o aprendizado de máquina, que, juntamente
		com as técnicas de descobrimento de conhecimento, dão origem às técnicas
		de mineração de dados. Finalmente, apresenta-se a mineração Web de
		conteúdo (ou Web Content Mining\nomenclature[WCM]{WCM}{Web Content Mining})
		...
		mineração de conteúdo de outros sites nos quais também são oferecidos
		...
		de performance entre as redes neurais Radial Basis Function (RBF)
		e Support Vector Machine com o método Naive Bayes. Utiliza-se, na
		aplicação Web proposta, a rede neural RBF para poder minerar os web
		logs da Wikipédia. Não se tem uma visão definida sobre a rede neural
		a ser projetada porque o problema a ser resolvido não foi analisado
		com detalhe.
		
		No artigo acadêmico intitulado \emph{“E-commerce website ranking using
			Semantic Web mining and neural computing”} referenciado em \cite{verma_2015},
		...
		páginas Web de comércio eletrônico. A implementação das técnicas de
		mineração Web de uso e de conteúdo no presente trabalho, são inspiradas
		nas técnicas propostas neste artigo.
		
		No artigo acadêmico intitulado \emph{“A framework for building Web
			mining applications in the world of blogs: A case study in product
			sentiment analysis”} referenciado em \cite{costa_2012}, desenvolve-se
		um framework implementado na linguagem Java™ que utiliza os serviços
		...
		dados na Web (Web \emph{crawler}) e um extrator de textos. No presente
		trabalho, o Web \emph{crawler} e o extrator de textos serão implementados.
		Por outro lado, a interface de usuário será desenvolvida para que
		seja executada na Web.
		
		No artigo acadêmico intitulado \emph{“Web Classification Mining Based
			on Radial Basis Probabilistic Neural Network”} referenciado em \cite{gao_2009},
		utiliza-se o método K-Nearest Neighbor (KNN) modificado para treinar
		a rede neural do tipo Radial Basis Probabilistic; os dados utilizados
		para classificação e treinamento foram extraídos de mil páginas Web
		utilizando a mineração Web de conteúdo. Será utilizado, no presente
		trabalho, o método KNN para poder agrupar os dados dos Web logs da
		Wikipédia.
		
		No artigo acadêmico intitulado \emph{“Structured Web information extraction
			using repetitive subject pattern”} referenciado em \cite{thamviset_2012}
		...
		por um extrator de dados e um \emph{wrapper} que traduz os conteúdos
		da Web para tabelas do tipo relacionais. O extrator de dados das páginas
		...
		relacionados com a monografia, utilizando-se uma abordagem top-down
		como pode-se observar na \cref{fig:organ_teorica}, e exibe-se uma
		rede de Petri que representa a montagem teórica do presente trabalho,
		na qual o estado inicial é o mais genérico e o estado final é o mais
		específico. 
		...
		\begin{figure}[h]
			...
			\includegraphics[scale=0.61]{img/rede_petri_teoria}\\\vspace{-0.1em}\fonteProp
		\end{figure}
		...
		as quais serão adotadas para que a ficha possa ativar as transições.
		...
		A SC é composta por técnicas como as redes neurais (Neural
		Networks \nomenclature[NN]{NN}{Neural Networks}), computação evolucionária
		(Evolutionary Computing\nomenclature[EC]{EC}{Evolutionary Computing}), sistemas difusos (Fuzzy Systems\nomenclature[FS]{FS}{Fuzzy Systems})
		...
		explica-se que mineração na Web trata do processo de descoberta de
		informação útil e relevante a partir de estruturas de hiper-referências,
		conteúdo das páginas e dados de uso de clientes. Justamente essas
		três fontes de extração de informação fazem referência às três categorias
		de mineração na Web mostradas a seguir, conforme a \cref{fig:Web_mining_taxonomia}:
		...
		\begin{description}[style=multiline,leftmargin=2.5cm]
			...
			\item [{Estrutura}] Ou Web Structure Mining (WSM\nomenclature[WSM]{WSM}{Web
			Structure Mining}), nesta técnica considera-se uma página Web como um nó e as referências
		...
		e o segundo fator, é a utilização de métodos de Web \emph{scraping} que se
		...
		utilizadas para obter informação diretamente de páginas Web utilizando requisições
		HTTP.
		...
		Esta seção não tem como intenção explicar sobre as funcionalidades
		e recursos básicos da linguagem Python, mas a intenção é expor sobre
		as bibliotecas necessárias que se utilizam para conseguir o objetivo
		estabelecido na presente monografia.
		
		Scrapy, Requests e Beautiful Soup são três bibliotecas \emph{open source}
		...
		bibliotecas é mostrado a seguir:
		
		\begin{description}[style=multiline,leftmargin=3.3cm]
			
			\item [{Scrapy}] É um framework escrito em Python para fazer rastreadores
			Web (Web \emph{crawling}) mediante a criação de “\emph{spiders}”, que são
			robôs que coletam informação de uma determinada página Web.
		...
		observe a \cref{fig:arquivo_wiki}, utilizam-se as seguintes bibliotecas
		\emph{open source}: SciPy, NumPy, Matplotlib e Jupyter. Uma breve explicação
		sobre o funcionamento dessas bibliotecas é exposta a seguir:
		...
		\begin{description}[style=multiline,leftmargin=2.5cm]
			...
			em duas dimensões. Utiliza-se geralmente com a feramenta Jupyter para
		...
		Python utilizando o framework Django. São implementadas, na plataforma,
		as rotinas para a obtenção de dados, pré-processamento, agrupamento,
		...
		Python (versão 2.7) com as suas respectivas bibliotecas para realizar
		...
		\begin{description}[style=multiline,leftmargin=4cm]
			...
			artigos acadêmicos dos usuários baseando-se na mineração de Web logs
			...
			\item [{Análise dos dados}] Os dados vem de duas fontes: Web logs
			...
			bancos de dados de Web logs da Wikipédia e os que foram obtidos através 
			da mineração Web de modo que o “ruído” não desejado é eliminado.
			Também aplicam-se técnicas de agrupamento de dados para
			poder otimizar o treinamento da rede neural que é modelada no 
			passo seguinte.
		...
		da aplicação Web proposta no trabalho, conforme se observa na
		\cref{fig:petri_funcionamento}.
		...
		em vários sites (relacionados com artigos acadêmicos), e
		é realizada utilizando os métodos de Web \emph{scraping}. Para extrair dados
		da Web são utilizados os algoritmos Subject Detection e Node Density,
		propostos em \cite{petprasit_2015}. Em seguida, aplica-se a técnica
		de mineração Web de conteúdo. Os dados obtidos a partir da mineração
		Web são armazenados em um banco de dados não relacional (neste caso, o
		MongoDB) e analisam-se, juntamente com os dados dos Web logs da Wikipédia,
		utilizando uma técnica de rede neural com aprendizagem supervisionada.
		Finalmente, a resposta preparada é mostrada para o usuário. Tal resposta
		contém links de páginas Web de artigos acadêmicos que estão relacionados
		com as palavras-chave da busca.
		...
		realizar todas as tarefas. Lembrando que o processo CRISP-DM
		...
		\begin{table}[H]
			...
			\begin{tabular}{>{\centering}m{5.3cm}>{\centering}m{5.5cm}}
				...
				\midrule
				\midrule 
				Compreensão do projeto  & 01/06/2016 $\longrightarrow$
				14/07/2016\tabularnewline
				\midrule
				Análise dos dados  & 15/07/2016 $\longrightarrow$ 02/08/2016\tabularnewline
				\midrule
				Preparação dos dados  & 03/08/2016 $\longrightarrow$ 14/08/2016\tabularnewline
				\midrule
				Modelagem, avaliação, desenvolvimento e TDD  & 15/08/2016 $\longrightarrow$
				15/09/2016\tabularnewline
				\bottomrule
